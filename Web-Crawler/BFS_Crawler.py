...
def main():
    ...
    open('bfs-logger.txt', 'a').write("Total length of URL set: " + str(len(total_hyper_links)))

    # Print the contents of the DFS Search to a file
    print_to_file(total_hyper_links, "bfs-urls.txt")
...
def process():
    ...
    with open("BFS_HTML_FILES/%s.html" % title.string, 'wb+') as file:
...
def dfs_search(start):
    ...
    for d in range(1, _DEPTH + 1):
        # Debug Information
        open('bfs-logger.txt', 'a').write("Depth: " + str(d) + "\n")
        ...
        while depth[str(d)] and len(total_hyper_links) < _MAX_URLS:
            ...
            # Debug Information
            open('bfs-logger.txt', 'a').write("Current Stack: ")
            for m in stack:
                open('bfs-logger.txt', 'a').write(m)
