...
void display(struct GBN_Queue *q){
	...
	struct GBN_Queue *m;
	//printf("sequence in the node are: ");
	while(n != NULL){
...
struct GBN_Queue* resend(struct GBN_Queue *, int, long, char*, char*, int, int);
void sendSyncPacket(int, char*, char*);
...
void  send_N_packets(int sockID, int last_sequence_number, int file_size, int messageType, FILE* input_file, int window_size, char* address, char* port){
	...
	while(1){
		if(messageType == INIT || messageType == ACK || messageType == ALLOK){
			...
			sendSyncPacket(sockID, address, port);
		}
		...
		// of the window
		if(messageType == ALLOK){
			last_sequence_number = curr_sequence_number;
			free(queue);
			queue = create_queue();
		} else if(messageType == ACK && curr_sequence_number != last_sequence_number){
			continue;
		} else if(messageType == NOTALLOK){
			...
			while(1){
				...
				currAddress = data_received_buffer;
				memcpy(&messageType, currAddress, sizeof(int));							//Get data-type from the acknkowledgment message
				//printf("in not ok loop: messageType %d\n",messageType);
				currAddress = currAddress + sizeof(int);
				memcpy(&last_sequence_number, currAddress, sizeof(int));		//Get sequence-number of last packet from the acknkowledgment message
				currAddress += sizeof(int);
				...
				if(messageType == ALLOK){
					...
					queue = resend(queue, sockID, bufferSize, address, port, window_size, last_sequence_number);
				}
			}
		}
...
void sendSyncPacket(int sockID, char* address, char* port){
	char dataP[PACKET_SIZE];
	char* currAddress;
	int messageType;
	...
	memcpy(currAddress, &ALLOK, sizeof(int));
	currAddress += sizeof(int);

	send_packet(sockID, dataP, PACKET_SIZE, address, port);
	//printf("Sent all ok packet\n");
}

struct GBN_Queue* resend(struct GBN_Queue *queue, int sockID, long bufferSize, char* address, char* port, int window_size, int sequence){
	//Variables
...
char* send_packet(int sockID, char* data, long bufferSize, char* address, char* port){
  ...
  ip4addr.sin_family = AF_INET;
	...
	// if(stop_flag < 2 && (incrementer == 2 || incrementer == 7)){
	// 	++stop_flag;
	// 	++incrementer;
...
char* read_packet(int sockID, char* readMessage, struct GBN_Queue *q, long bufferSize, char* address, char* port, int window_size){
	...
	while(retry <= 4){
		...
		if(status == 0 && retry <= 3){
				...
				if(q != NULL)
					queue = resend(q, sockID, bufferSize, address, port, window_size, -1);
